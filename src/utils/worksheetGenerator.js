...
/**
 * Export the translated worksheet HTML as a PPTX file.
 * Each A4-proportioned page slice becomes one 10×7.5-inch slide (image fill).
 */
export async function downloadAsPptxFromHtml(html, filename = 'worksheet') {
  const { css, body } = parseHtmlDoc(html)
  ...
  const RENDER_W_PX = Math.round(794 * (CONTENT_W_MM / A4_W_MM))
  ...
  try {
    ...
    // Slide dimensions in inches (widescreen 10×7.5 matches standard PPT)
    const SLIDE_W_IN = 10
    ...
    pptx.defineLayout({ name: 'A4', width: SLIDE_W_IN, height: SLIDE_H_IN })
    pptx.layout = 'A4'
    ...
    while (srcY < canvas.height) {
      ...
      slide.addImage({
        ...
        w     : SLIDE_W_IN,
        h     : SLIDE_H_IN,
        sizing: { type: 'contain', w: SLIDE_W_IN, h: SLIDE_H_IN },
      })
